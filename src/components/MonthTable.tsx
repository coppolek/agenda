              ...
              {days.map(d => {
                ...
                <th key={d.toISOString()} className={cn(
                  ...
                  <div className="flex flex-col items-center gap-1">
                    <span className={cn("font-bold", (isHoliday || isSunday) ? "text-red-700" : isSaturday ? "text-sky-700" : "text-slate-700")}>{format(d, 'd')}</span>
                    <span className={cn("text-[10px] uppercase font-semibold", (isHoliday || isSunday) ? "text-red-500" : isSaturday ? "text-sky-500" : "text-slate-400")}>{format(d, 'EE', { locale: it })[0]}</span>
                  </div>
                </th>
              )})}
              <th className={cn(
